...
TEST_CASE("common::logger", "[common][logger]") {
  auto logger =
      phansar::common::logger{"unittest",
                              "logs/unittest.log",
  ...
  logger.log(
      spdlog::level::trace, "trace", spdlog::source_loc{"<none>", 1, ""});
  logger.log(
      spdlog::level::debug, "debug", spdlog::source_loc{"<none>", 1, ""});
  logger.log(spdlog::level::info, "info", spdlog::source_loc{"<none>", 1, ""});
  logger.log(
      spdlog::level::warn, "warning", spdlog::source_loc{"<none>", 1, ""});
  logger.log(spdlog::level::err, "error", spdlog::source_loc{"<none>", 1, ""});
  logger.log(
      spdlog::level::critical, "critical", spdlog::source_loc{"<none>", 1, ""});
  ...
  for (auto i = std::size_t{0}; i < std::thread::hardware_concurrency(); ++i) {
    threads.emplace_back([&logger]() {
      ...
      logger.log(
          spdlog::level::debug, "debug", spdlog::source_loc{"<none>", 1, ""});
      logger.log(
          spdlog::level::info, "info", spdlog::source_loc{"<none>", 1, ""});
      logger.log(
          spdlog::level::warn, "warning", spdlog::source_loc{"<none>", 1, ""});
      logger.log(
          spdlog::level::err, "error", spdlog::source_loc{"<none>", 1, ""});
