...
void sol_visitor::visit_type_begin(const rttr::visitor::type_info<T> & _info) {
    using declaring_type_t = typename std::remove_cvref_t<decltype(_info)>::declaring_type;
...
void sol_visitor::visit_type_end(const rttr::visitor::type_info<T> & _info) {
    using declaring_type_t = typename std::remove_cvref_t<decltype(_info)>::declaring_type;
    auto usertype = _state_internal()[rttr::type::get<declaring_type_t>().get_name().to_string()]
...
void sol_visitor::visit_constructor(const rttr::visitor::constructor_info<T> & _info) {
    using declaring_type_t = typename std::remove_cvref_t<decltype(_info)>::declaring_type;
    auto usertype = _state_internal()[rttr::type::get<declaring_type_t>().get_name().to_string()]
                        .template get<sol::usertype<declaring_type_t>>();
    HEDLEY_DIAGNOSTIC_PUSH
    ...
    usertype[fmt::format("new{}", _constructor_counter_next_internal())] =
        [](const CtorArgs &... _args) { return std::make_unique<declaring_type_t>(_args...); };
    HEDLEY_DIAGNOSTIC_POP
}
...
void sol_visitor::visit_constructor_function(
    ...
    auto usertype = _state_internal()[rttr::type::get<declaring_type_t>().get_name().to_string()]
                        .template get<sol::usertype<declaring_type_t>>();
    usertype[fmt::format("new{}", _constructor_counter_next_internal())] = _info.function_ptr;
}
...
template <class T>
void sol_visitor::visit_global_method(const rttr::visitor::method_info<T> & _info) {
    visit_method(_info);
}

template <class T>
void sol_visitor::visit_property(const rttr::visitor::property_info<T> & _info) {
    using declaring_type_t = typename std::remove_cvref_t<decltype(_info)>::declaring_type;
    auto usertype = _state_internal()[rttr::type::get<declaring_type_t>().get_name().to_string()]
                        .template get<sol::usertype<declaring_type_t>>();
    if constexpr (std::is_member_object_pointer_v<decltype(_info.property_accessor)>) {
        usertype[_info.property_item.get_name().to_string()] = _info.property_accessor;
    } else {
...
template <class T>
void sol_visitor::visit_global_property(const rttr::visitor::property_info<T> & _info) {
...
template <class T>
void sol_visitor::visit_readonly_property(const rttr::visitor::property_info<T> & _info) {
    using declaring_type_t = typename std::remove_cvref_t<decltype(_info)>::declaring_type;
    auto usertype = _state_internal()[rttr::type::get<declaring_type_t>().get_name().to_string()]
                        .template get<sol::usertype<declaring_type_t>>();
    if constexpr (std::is_member_object_pointer_v<decltype(_info.property_accessor)>) {
    ...
    } else if constexpr (std::is_member_function_pointer_v<decltype(_info.property_accessor)>) {
        usertype[_info.property_item.get_name().to_string()] =
            sol::property(_info.property_accessor);
    } else {
        usertype[_info.property_item.get_name().to_string()] = sol::var(*_info.property_accessor);
    }
}

template <class T>
void sol_visitor::visit_global_readonly_property(const rttr::visitor::property_info<T> & _info) {
...
void sol_visitor::visit(const rttr::type & _type) {
    if constexpr (std::is_enum_v<T>) {
        ...
        auto usertype =
            _state_internal().new_usertype<T>(rttr::type::get<T>().get_name().to_string(),
                                              sol::no_constructor);

        auto enumeration = _type.get_enumeration();
        for (const auto & name : enumeration.get_names()) {
            bool ok = false;
            usertype[name.to_string()] =
                sol::var(static_cast<T>(enumeration.name_to_value(name).to_int64(&ok)));
            PH_ASSERT(ok);
        }
    } else {
