...
synchronized<T, ImplementationPolicy>::proxy<Writable>::proxy(
    T & _obj, std::shared_mutex & _mutex)
    : m_impl{&_obj, &_mutex} {}
...
template <bool Writable>
template <bool Writable2, class>
auto synchronized<T, ImplementationPolicy>::proxy<Writable>::get() const
    -> const T * {
  return m_impl->obj;
}
...
template <bool Writable>
template <bool Writable2, class>
auto synchronized<T, ImplementationPolicy>::proxy<Writable>::operator*() const
    -> const T & {
  return *get();
}
...
template <class T, class ImplementationPolicy>
template <class... Args>
constexpr synchronized<T, ImplementationPolicy>::synchronized(Args &&... _args)
    : m_impl{T{std::forward<Args>(_args)...},
...
template <class T, class ImplementationPolicy>
synchronized<T, ImplementationPolicy>::synchronized(
    synchronized && _other) noexcept = default;

template <class T, class ImplementationPolicy>
auto synchronized<T, ImplementationPolicy>::operator=(
    synchronized && _other) noexcept
    -> synchronized<T, ImplementationPolicy> & = default;
