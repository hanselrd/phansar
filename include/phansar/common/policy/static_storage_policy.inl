...
static_storage_policy<T, Size, Align>::static_storage_policy(Args &&... _args) {
    static_assert(Size >= sizeof(T), "Size must be greater than or equal to sizeof(T)");
    static_assert(Size <= sizeof(T) * 1.1,
                  "Size must be less than or equal to sizeof(T) with a 10% tolerance margin");
...
auto static_storage_policy<T, Size, Align>::operator*() -> T & {
    return const_cast<T &>(static_cast<const static_storage_policy &>(*this).operator*());
}
