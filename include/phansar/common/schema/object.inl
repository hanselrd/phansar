...
object<T>::object(capnp::MallocMessageBuilder * _message) {
  if (_message != nullptr) {
    ...
    m_impl->builder = std::get<capnp::MallocMessageBuilder *>(m_impl->message)
                          ->template getRoot<T>();
  } else {
    m_impl->message
        .template emplace<std::unique_ptr<capnp::MallocMessageBuilder>>(
            new capnp::MallocMessageBuilder{});
    m_impl->builder =
        std::get<std::unique_ptr<capnp::MallocMessageBuilder>>(m_impl->message)
            ->template getRoot<T>();
  }
}
...
object<T>::object(std::vector<kj::byte> const & _bytes,
    ...
    : object{_message} {
  auto stream =
      kj::ArrayInputStream{kj::arrayPtr(_bytes.data(), _bytes.size())};
  auto reader = capnp::PackedMessageReader{stream};

  if (std::holds_alternative<std::unique_ptr<capnp::MallocMessageBuilder>>(
          m_impl->message)) {
    auto & message =
        std::get<std::unique_ptr<capnp::MallocMessageBuilder>>(m_impl->message);
    message->setRoot(reader.getRoot<T>());
    m_impl->builder = message->template getRoot<T>();
  } else if (std::holds_alternative<capnp::MallocMessageBuilder *>(
                 m_impl->message)) {
...
template <class T>
object<T>::object(capnp::ReaderFor<T>           _reader,
                  capnp::MallocMessageBuilder * _message)
    : object{_message} {
  if (std::holds_alternative<std::unique_ptr<capnp::MallocMessageBuilder>>(
          m_impl->message)) {
...
template <class T>
object<T>::object(capnp::BuilderFor<T>          _builder,
                  capnp::MallocMessageBuilder * _message) {
  if (_message != nullptr) {
    m_impl->message.template emplace<capnp::MallocMessageBuilder *>(_message);
  }
...
auto object<T>::is_self_contained() const -> bool {
  return std::holds_alternative<std::unique_ptr<capnp::MallocMessageBuilder>>(
      m_impl->message);
}
