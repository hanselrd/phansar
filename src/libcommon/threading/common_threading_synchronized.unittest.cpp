...
TEST_CASE("common_threading_synchronized", "[common][threading][synchronized]") {
    auto threads = std::vector<std::thread>{};

    SECTION("can use synchronized vector") {
        ...
        REQUIRE(s.lock()->size() == (THREADING_SYNCHRONIZED_UNITTEST_MAX_ITERATIONS *
                                     std::thread::hardware_concurrency()));
    }

    SECTION("can use synchronized struct") {
        struct point {
            int x;
            int y;
        };
    ...
    SECTION("can use synchronized primitive") {
