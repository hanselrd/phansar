...
template <class T> void load(const std::string &path) {
    ...
    if (typeid(T) == typeid(common::core::json)) {
        ...
        internal::load(path, internal::resource_type::MIX_MUSIC);
    } else if (typeid(U) == typeid(std::shared_ptr<TTF_Font>)) {
        internal::load(path, internal::resource_type::TTF_FONT);
    }
}
...
template <class T> std::shared_ptr<T> get(const std::string &path) {
    ...
    if (typeid(T) == typeid(common::core::json)) {
        ...
        return std::static_pointer_cast<T>(
            internal::get(path, internal::resource_type::SDL_SURFACE));
    } else if (typeid(U) == typeid(std::shared_ptr<Mix_Music>)) {
        return std::static_pointer_cast<T>(internal::get(path, internal::resource_type::MIX_MUSIC));
    } else if (typeid(U) == typeid(std::shared_ptr<TTF_Font>)) {
        return std::static_pointer_cast<T>(internal::get(path, internal::resource_type::TTF_FONT));
    }
