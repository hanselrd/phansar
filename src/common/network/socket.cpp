...
struct socket::impl {
    ...
    std::unordered_map<channel, std::queue<std::pair<std::uint16_t, std::vector<std::uint8_t>>>>
        queues{};
};
...
void socket::poll(std::uint32_t _timeout) {
    ...
    while (enet_host_service(m_impl->host, &event, _timeout) > 0) {
        ...
        case ENET_EVENT_TYPE_RECEIVE:
            if (get_type() == type::server) {
                PH_LOG_DEBUG("Received packet from client {} on channel {} [{} @ {} byte(s)]",
                             event.peer->incomingPeerID,
                             event.channelID,
                             static_cast<void *>(event.packet->data),
                             event.packet->dataLength);
            } else {
                PH_LOG_DEBUG("Received packet from server on channel {} [{} @ {} byte(s)]",
                             event.channelID,
                             static_cast<void *>(event.packet->data),
                             event.packet->dataLength);
            }

            m_impl->queues[static_cast<channel>(event.channelID)].push(std::make_pair(
                event.peer->incomingPeerID,
                std::vector<std::uint8_t>{event.packet->data,
...
void socket::send(std::uint16_t        _client_id,
    ...
    auto * packet = enet_packet_create(_ptr, _size, _channel_to_packet_flags_internal(_channel));

    enet_peer_send(peer, static_cast<std::underlying_type_t<channel>>(_channel), packet);
    enet_host_flush(m_impl->host);
}

void socket::broadcast(channel _channel, const std::uint8_t * _ptr, std::size_t _size) {
    PH_ASSERT(m_impl->host);
...
void socket::_open_internal() {
    m_impl->host = enet_host_create((get_type() == type::server) ? &m_impl->address : nullptr,
...
auto socket::_channel_to_packet_flags_internal(channel _channel) const -> std::uint32_t {
