...
public:
  void format(const spdlog::details::log_msg & _msg,
    ...
    if (! s_thread_name_map.lock_shared()->contains(
            std::this_thread::get_id())) {
      auto write_lock = s_thread_name_map.lock();
      if (write_lock->empty()) {
        write_lock->insert_or_assign(
            std::this_thread::get_id(),
    ...
    auto thread_name =
        s_thread_name_map.lock_shared()->at(std::this_thread::get_id());
...
logger::logger(std::string_view _name,
               ...
               std::size_t      _size,
               std::size_t      _count) {
  m_impl->stdout_color_sink =
      std::make_shared<spdlog::sinks::stdout_color_sink_mt>();
  m_impl->rotating_file_sink =
      std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
          std::string{_path}, _size, _count);
  m_impl->logger = std::make_shared<spdlog::logger>(
      std::string{_name},
      spdlog::sinks_init_list{m_impl->stdout_color_sink,
                              m_impl->rotating_file_sink});
  ...
  rttr::registration::class_<logger>("logger")
      ...
      .method("trace",
              [](logger & _obj, std::string_view _message) {
                _obj.log(spdlog::level::trace,
                         _message,
                         spdlog::source_loc{"<none>", 1, ""});
              })
      .method("debug",
              [](logger & _obj, std::string_view _message) {
                _obj.log(spdlog::level::debug,
                         _message,
                         spdlog::source_loc{"<none>", 1, ""});
              })
      .method("info",
              [](logger & _obj, std::string_view _message) {
                _obj.log(spdlog::level::info,
                         _message,
                         spdlog::source_loc{"<none>", 1, ""});
              })
      .method("warning",
              [](logger & _obj, std::string_view _message) {
                _obj.log(spdlog::level::warn,
                         _message,
                         spdlog::source_loc{"<none>", 1, ""});
              })
      .method("error",
              [](logger & _obj, std::string_view _message) {
                _obj.log(spdlog::level::err,
                         _message,
                         spdlog::source_loc{"<none>", 1, ""});
              })
      .method("critical", [](logger & _obj, std::string_view _message) {
        _obj.log(spdlog::level::critical,
                 _message,
                 spdlog::source_loc{"<none>", 1, ""});
      });
