function(ph_add_file_pass)
  ...
  if(ARG_PRE_COMMANDS)
    ...
    list(TRANSFORM pre_commands
         REPLACE "@TMP_DIR@" "${CMAKE_BINARY_DIR}/file-pass/${ARG_NAME}")
    ...
    add_custom_target(
      ...
      COMMAND ${CMAKE_COMMAND} -E make_directory
              ${CMAKE_BINARY_DIR}/file-pass/${ARG_NAME}
      COMMAND
        ${CMAKE_COMMAND} -DCOMMANDS:STRING="${pre_commands}"
        -DWORKING_DIRECTORY:STRING=${CMAKE_SOURCE_DIR}
        -DREDIRECT_OUTPUT:BOOL=${ARG_PRE_REDIRECT_OUTPUT}
        -DSEQUENTIAL:BOOL=${ARG_PRE_SEQUENTIAL} -P
        ${CMAKE_SOURCE_DIR}/cmake/scripts/execute-process.cmake
  ...
  foreach(file IN LISTS files)
    ...
    list(TRANSFORM commands REPLACE "@INPUT_LAST_EXTENSION@"
                                    "${last_extension}")
    ...
    add_custom_target(
      intra-${ARG_NAME}-${hash}
      COMMAND
        ${CMAKE_COMMAND} -DCOMMANDS:STRING="${commands}"
        -DWORKING_DIRECTORY:STRING=${CMAKE_SOURCE_DIR}
        -DREDIRECT_OUTPUT:BOOL=${ARG_REDIRECT_OUTPUT}
        -DSEQUENTIAL:BOOL=${ARG_SEQUENTIAL} -P
        ${CMAKE_SOURCE_DIR}/cmake/scripts/execute-process.cmake
  ...
  if(ARG_POST_COMMANDS)
    ...
    list(TRANSFORM post_commands
         REPLACE "@TMP_DIR@" "${CMAKE_BINARY_DIR}/file-pass/${ARG_NAME}")

    add_custom_target(
      post-${ARG_NAME}
      COMMAND
        ${CMAKE_COMMAND} -DCOMMANDS:STRING="${post_commands}"
        -DWORKING_DIRECTORY:STRING=${CMAKE_SOURCE_DIR}
        -DREDIRECT_OUTPUT:BOOL=${ARG_POST_REDIRECT_OUTPUT}
        -DSEQUENTIAL:BOOL=${ARG_POST_SEQUENTIAL} -P
        ${CMAKE_SOURCE_DIR}/cmake/scripts/execute-process.cmake
...
if(ENABLE_CLANG_FORMAT)
  ...
  if(CLANG_FORMAT_EXECUTABLE)
    ph_add_file_pass(
      ...
      GROUPS check-format
      COMMANDS
        "${CLANG_FORMAT_EXECUTABLE} --style=file --dry-run --Werror @INPUT_FILE@"
      GLOBS "include/*.hpp"
            "include/*.inl"
            "include/*.cpp"
            "src/*.hpp"
...
if(ENABLE_CLANG_TIDY)
  ...
  if(CLANG_TIDY_EXECUTABLE AND CLANG_APPLY_REPLACEMENTS_EXECUTABLE)
    ...
    ph_add_file_pass(
      ...
      POST_COMMANDS
        "${CLANG_APPLY_REPLACEMENTS_EXECUTABLE} -format -style=file @TMP_DIR@"
      GLOBS "include/*.cpp"
            "src/*.cpp"
            "src/*.cpp.in"
            "test/*.cpp"
