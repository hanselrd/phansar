...
SAMPLER2D(s_material_emission, 2);
...
vec3 calculate_directional_light(vec3 _light_direction, vec3 _light_ambient, vec3 _light_diffuse, vec3 _light_specular, vec3 _position, vec2 _texcoord, vec3 _normal) {
    ...
    vec3 result = (_light_ambient * u_material_ambient.xyz * texture2D(s_material_diffuse, _texcoord).rgb) +
                  (_light_diffuse * u_material_diffuse.xyz * diffuse * texture2D(s_material_diffuse, _texcoord).rgb) +
                  (_light_specular * u_material_specular.xyz * specular * texture2D(s_material_specular, _texcoord).rgb);

    return result;
}

vec3 calculate_point_light(vec3 _light_position, vec3 _light_ambient, vec3 _light_diffuse, vec3 _light_specular, float _light_attenuation_kc, float _light_attenuation_kl, float _light_attenuation_kq, vec3 _position, vec2 _texcoord, vec3 _normal) {
    ...
    vec3 result = (_light_ambient * u_material_ambient.xyz * texture2D(s_material_diffuse, _texcoord).rgb * attenuation) +
                  (_light_diffuse * u_material_diffuse.xyz * diffuse * texture2D(s_material_diffuse, _texcoord).rgb * attenuation) +
                  (_light_specular * u_material_specular.xyz * specular * texture2D(s_material_specular, _texcoord).rgb * attenuation);

    return result;
}

vec3 calculate_spot_light(vec3 _light_position, vec3 _light_direction, vec3 _light_ambient, vec3 _light_diffuse, vec3 _light_specular, float _light_attenuation_kc, float _light_attenuation_kl, float _light_attenuation_kq, float _light_cutoff_inner, float _light_cutoff_outer, vec3 _position, vec2 _texcoord, vec3 _normal) {
    vec3 light_positionView = mul(u_view, vec4(_light_position, 0.0)).xyz;
    vec3 light_directionView = mul(u_view, vec4(_light_direction, 0.0)).xyz;
    vec3 light_dir = normalize(light_positionView - _position);
    vec3 view_dir = normalize(-_position);
    ...
    vec3 result = (_light_ambient * u_material_ambient.xyz * texture2D(s_material_diffuse, _texcoord).rgb * attenuation * intensity) +
                  (_light_diffuse * u_material_diffuse.xyz * diffuse * texture2D(s_material_diffuse, _texcoord).rgb * attenuation * intensity) +
                  (_light_specular * u_material_specular.xyz * specular * texture2D(s_material_specular, _texcoord).rgb * attenuation * intensity);
...
void main() {
    ...
    for (int i = 0; i < DIRECTIONAL_LIGHT_COUNT; ++i) {
        result += calculate_directional_light(u_directional_light_direction[i].xyz, u_directional_light_ambient[i].xyz, u_directional_light_diffuse[i].xyz, u_directional_light_specular[i].xyz, v_position, v_texcoord0, v_normal);
    }

    for (int i = 0; i < POINT_LIGHT_COUNT; ++i) {
        result += calculate_point_light(u_point_light_position[i].xyz, u_point_light_ambient[i].xyz, u_point_light_diffuse[i].xyz, u_point_light_specular[i].xyz, u_point_light_attenuation_kc(i), u_point_light_attenuation_kl(i), u_point_light_attenuation_kq(i), v_position, v_texcoord0, v_normal);
    }

    for (int i = 0; i < SPOT_LIGHT_COUNT; ++i) {
        result += calculate_spot_light(u_spot_light_position[i].xyz, u_spot_light_direction[i].xyz, u_spot_light_ambient[i].xyz, u_spot_light_diffuse[i].xyz, u_spot_light_specular[i].xyz, u_spot_light_attenuation_kc(i), u_spot_light_attenuation_kl(i), u_spot_light_attenuation_kq(i), u_spot_light_cutoff_inner(i), u_spot_light_cutoff_outer(i), v_position, v_texcoord0, v_normal);
    }

    gl_FragColor = vec4(result + (texture2D(s_material_emission, v_texcoord0).rgb * step(vec3_splat(1.0), vec3_splat(1.0) - texture2D(s_material_specular, v_texcoord0).rgb)), 1.0);
}
